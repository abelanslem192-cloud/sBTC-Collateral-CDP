...
(define-public (borrow (amount uint) (collateral-token <sip010-token>) (oracle <oracle-trait>))
    (let (
            (token-principal (contract-of collateral-token))
            (type-info (unwrap! (map-get? supported-collateral token-principal) err-collateral-not-supported))
            (vault (get-vault tx-sender token-principal))
...
(define-public (withdraw-collateral (amount uint) (collateral-token <sip010-token>) (oracle <oracle-trait>))
    (let (
            (token-principal (contract-of collateral-token))
            (type-info (unwrap! (map-get? supported-collateral token-principal) err-collateral-not-supported))
            (vault (get-vault tx-sender token-principal))
