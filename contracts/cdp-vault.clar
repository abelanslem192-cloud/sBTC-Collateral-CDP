...
(define-private (calculate-accrued-interest (debt uint) (last-accrued-block uint) (interest-rate uint))
...
(define-read-only (calculate-current-ratio (user principal) (token principal))
    (let (
            (vault (get-vault user token))
            (type-info (unwrap! (get-collateral-type token) u0))
            (collateral-val (calculate-collateral-value (get collateral vault) (get price type-info)))
            (pending-interest (calculate-accrued-interest (get debt vault) (get last-accrued-block vault) (get interest-rate type-info)))
...
(define-public (update-collateral-price (token principal) (new-price uint))
    (let (
            (type-info (unwrap! (map-get? supported-collateral token) err-collateral-not-supported))
        )
        (asserts! (is-eq tx-sender contract-owner) err-owner-only)
        (ok (map-set supported-collateral token (merge type-info { price: new-price })))
    )
)

;; User Functions

(define-public (deposit-collateral (amount uint) (collateral-token <sip010-token>))
    (let (
            ...
            (vault (get-vault tx-sender token-principal))
        ...
        (map-set vaults { owner: tx-sender, collateral-token: token-principal } {
...
(define-public (borrow (amount uint) (collateral-token <sip010-token>))
    (let (
            (token-principal (contract-of collateral-token))
            (type-info (unwrap! (map-get? supported-collateral token-principal) err-collateral-not-supported))
            (vault (get-vault tx-sender token-principal))
        ...
        (asserts! (get active type-info) err-collateral-not-supported)
        (asserts! (>= (/ (* collateral-val u100) new-debt) (get liquidation-ratio type-info)) err-under-collateralized)
...
(define-public (repay (amount uint) (collateral-token <sip010-token>))
    (let (
            (token-principal (contract-of collateral-token))
            (type-info (unwrap! (map-get? supported-collateral token-principal) err-collateral-not-supported))
            (vault (get-vault tx-sender token-principal))
...
(define-public (withdraw-collateral (amount uint) (collateral-token <sip010-token>))
    (let (
            (token-principal (contract-of collateral-token))
            (type-info (unwrap! (map-get? supported-collateral token-principal) err-collateral-not-supported))
            (vault (get-vault tx-sender token-principal))
            ...
            (pending-interest (calculate-accrued-interest (get debt vault) (get last-accrued-block vault) (get interest-rate type-info)))
            (current-debt (+ (get debt vault) pending-interest))
            (collateral-val (calculate-collateral-value new-collateral (get price type-info)))
        )
        (asserts! (>= current-collateral amount) err-insufficient-collateral)
        (asserts!
            (or (is-eq current-debt u0) (>= (/ (* collateral-val u100) current-debt) (get liquidation-ratio type-info)))
            err-under-collateralized
        )

        (try! (as-contract (contract-call? collateral-token transfer amount tx-sender tx-sender none)))
...
(define-public (partial-liquidate (target principal) (collateral-token <sip010-token>))
    (let (
            (token-principal (contract-of collateral-token))
            (type-info (unwrap! (map-get? supported-collateral token-principal) err-collateral-not-supported))
            ...
            (pending-interest (calculate-accrued-interest (get debt vault) (get last-accrued-block vault) (get interest-rate type-info)))
            (current-debt (+ (get debt vault) pending-interest))
            (collateral-val (calculate-collateral-value collateral (get price type-info)))
        ...
        (if (< current-ratio critical-ratio)
            (liquidate-full target collateral-token)
            (let (
                    (target-debt (/ (* collateral-val u100) target-ratio-after-liquidation))
                    (debt-to-repay (- current-debt target-debt))
                    (debt-value-in-collateral (/ (* debt-to-repay u100000000) (get price type-info)))
                ...
                (try! (as-contract (contract-call? collateral-token transfer reward-collateral tx-sender tx-sender none)))
...
(define-public (liquidate (target principal) (collateral-token <sip010-token>))
    (liquidate-full target collateral-token)
)

(define-private (liquidate-full (target principal) (collateral-token <sip010-token>))
    (let (
            (token-principal (contract-of collateral-token))
            (type-info (unwrap! (map-get? supported-collateral token-principal) err-collateral-not-supported))
            (vault (get-vault target token-principal))
        ...
        (let (
                (debt-value-in-collateral (/ (* debt u100000000) (get price type-info)))
                (reward-collateral (/ (* debt-value-in-collateral (+ u100 liquidation-penalty)) u100))
