...
(define-private (liquidate-full
    ...
    (let (
        ...
        (let (
                ...
                ;; Adjust if capped by collateral
                (adjusted-keeper-payout (if (> (+ keeper-payout protocol-payout) collateral)
                    (- collateral protocol-payout)
                    keeper-payout
                ))
                (final-keeper-payout (if (> total-reward collateral)
                    collateral
                    keeper-payout
                ))
                (final-protocol-payout (if (> total-reward collateral)
                    u0
                    protocol-payout
                ))
            )
            ...
            (try! (as-contract (contract-call? collateral-token transfer final-keeper-payout
                tx-sender liquidator none
            )))

            ;; Update Reserve
            (map-set stability-reserve token-principal
                (+ (default-to u0 (map-get? stability-reserve token-principal))
                    final-protocol-payout
                ))
        ...
        (if (< current-ratio critical-ratio)
            ...
            (let (
                ...
                (map-set stability-reserve token-principal
                    (+
                        (default-to u0
                            (map-get? stability-reserve token-principal)
                        )
                        protocol-share
                    ))
